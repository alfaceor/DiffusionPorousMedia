...
figR2, axR2 = plt.subplots(1, 2, figsize=(16,6))
axR2_a = axR2[0]
axR2_b = axR2[1]
...
for hr in range(len(strHradList)):
  ...
  legendDict[strHrad] = axR2_a.legend(lineas, etiquetas, bbox_to_anchor=(0.75,0.03), loc="upper left", title="$R = "+strHrad+", p_{D}$", prop={'size':10}, frameon=False, numpoints=1, ncol=2)
...
for hr in range(len(strHradList)):
  ...
  for b2b in range(len(strB2BList)):
    ...
    try:
      ...
      if flagPlot :
        lineas[b2b], = axR2_b. plot(sd.ttime*pB2B/Hrad**2., sd.DelR2/(pB2B*sd.ttime), **estilo)
  ...
  legendDict[strHrad] = axR2_b.legend(lineas, etiquetas, bbox_to_anchor=(0.75,0.03), loc="upper left", title="$R = "+strHrad+", p_{S}$", prop={'size':10}, frameon=False, numpoints=1, ncol=2)
