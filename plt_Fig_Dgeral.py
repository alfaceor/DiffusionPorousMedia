...
for hr in range(len(strHradList)):
  ...
  legendDict[strHrad] = axDiff.legend(lineas, etiquetas, bbox_to_anchor=(0.75,0.03), loc="upper left", title="$R = "+strHrad+", p_{D}$", prop={'size':20}, frameon=False, numpoints=1, ncol=2, columnspacing=0.2)
...
legendDict[ "10"].set_bbox_to_anchor((0.0, 1.0) , transform=None)
legendDict[ "20"].set_bbox_to_anchor((0.0, 0.70) , transform=None)
legendDict[ "40"].set_bbox_to_anchor((0.0, 0.5) , transform=None)
legendDict[ "80"].set_bbox_to_anchor((0.5, 0.25) , transform=None)
legendDict["160"].set_bbox_to_anchor((0.65, 0.5) , transform=None)

fig.tight_layout()

#figDiff.savefig("D_vs_pBB__pBF_"+ "VAR" +"__Hrad_"+strHrad+".pdf")
fig.savefig("Dgeral.pdf")
