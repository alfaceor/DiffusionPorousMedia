...
for b2b in range(len(strB2BList)):
  ...
  axR2.set_title("$ p_{FB} = "+lblF2B+" $, $ p_{BB} = "+lblB2B+" $"+", $R_H = "+strHrad+"$")
  ...
  leg = axR2.legend(handles[::-1], labels[::-1], bbox_to_anchor=(0.02,0.35), loc="lower left", title="$p_{BF}$", prop={'size':12}, frameon=False)
